...
VAULT_FILE = $(COMPOSE_DIR)/.env.example
...
setup: ## Decrypt .env from encrypted template (prompts for vault password)
	@if [ -f $(COMPOSE_DIR)/.env ]; then \
		echo "compose/.env already exists. Delete it first to regenerate from template."; \
	else \
		$(ANSIBLE_VAULT) decrypt $(VAULT_FILE) --output $(COMPOSE_DIR)/.env || exit 1; \
		echo "Created compose/.env from encrypted template."; \
	fi
	@echo "IMPORTANT: Edit compose/.env to set your passwords before starting services."

$(COMPOSE_DIR)/.env: $(VAULT_FILE)
	@$(ANSIBLE_VAULT) decrypt $(VAULT_FILE) --output $(COMPOSE_DIR)/.env --force || \
		(echo "Error: ansible-vault not available or wrong vault password." && exit 1)
	@echo "Created compose/.env from encrypted template."
